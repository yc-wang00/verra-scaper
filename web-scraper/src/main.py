    ...
    for id, name in zip(df["ID"], df["Name"]):
        ...
        try:
            ...
            if scrape_document_links:
                ...
                document_headers = ['VCS Registration Documents', 'VCS Pipeline Documents', 'VCS Issuance Documents', 'VCS Other Documents']

                document_groups = []
                for header in document_headers:
                    # Find the document group by its unique header
                    found_document_group = driver.find_elements(By.XPATH, f"//div[contains(@class,'card') and .//div[contains(@class,'card-header')][contains(text(),'{header}')]]")
                    
                    # Check that only one document group was found
                    assert len(found_document_group) == 1, f"Expected 1 document group for header {header}, but found {len(found_document_group)}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ds", "--disable-summary", dest="disable_summary", action="store_true", help="No scraping of the summary")
    parser.add_argument("-dd", "--disable-document", dest="disable_document", action="store_true", help="No scraping of the document links")
